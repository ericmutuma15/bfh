  ...
  return (
    <div className="min-h-screen bg-gray-50 text-gray-900 flex flex-col w-full max-w-full overflow-x-hidden">
      <nav className="sticky top-0 z-50 bg-white shadow-md flex flex-col md:flex-row items-center md:justify-center py-2 w-full max-w-full">
        ...
        {/* Icons with glowing underline and tooltip */}
        <div className="flex flex-1 justify-center gap-8 w-full max-w-full">
          {navLinks.map((link) => {
            ...
            return (
              <Link
                key={link.to}
                to={link.to}
                className="relative text-2xl md:text-3xl transition-transform transform hover:scale-110 group"
              >
                {link.icon}

                {/* Underline effect: persistent if active, animated on hover */}
                <span
                  className={`absolute -bottom-1 left-1/2 transform -translate-x-1/2 h-1 rounded-lg bg-gradient-to-r from-blue-400 via-blue-600 to-blue-400 transition-all duration-500 ease-in-out ${
                    isActive
                      ...
                      : 'w-0 group-hover:w-full group-hover:animate-pulse'
                  }`}
                />

                {/* Tooltip below underline on hover */}
                <span
